...
public class CarPriceEstimator {
	...
	public double getSalePrice(String makeAndModel, int yearManufactured, double milesDriven, 
		...
		double salePrice = getPrice(makeAndModel, yearManufactured); // 20000
		int currentYear = LocalDate.now().getYear();		
		int ageOfCar = currentYear - yearManufactured + 1; // 1
		...
		salePrice = ageOfCar <= 10 ? (salePrice - 0.05 * salePrice * ageOfCar) : salePrice * 0.1; 
		...
		if(!(airBagsCount >=2 && hasAbs && hasEbd)) { // 19000
		    salePrice -= 1000;
		}
